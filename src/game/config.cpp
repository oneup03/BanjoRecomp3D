...
static void push_stereo_config_to_renderer() {
    ...
    uint32_t convergence = static_cast<uint32_t>(std::get<double>(
        graphics_config.get_temp_option_value(banjo::configkeys::graphics::stereo_convergence)));
    ...
    recompui::renderer::set_stereo_config(banjo_to_rt64_stereo(mode), separation, convergence, hudDepth, autoConv, autoConvScale);
}
...
static void add_graphics_options(recomp::config::Config &config) {
    ...
    config.add_number_option(
        ...
        "Stereo Separation",
        "Controls the perceived depth strength (eye distance). Higher values produce a stronger 3D effect but can cause eye strain.",
        0, 100, 1, 0, false, 50
    );
    config.add_number_option(
        banjo::configkeys::graphics::stereo_convergence,
        "Stereo Convergence",
        "Sets the distance to the zero-parallax plane (where geometry sits exactly on the screen). Lower values push the scene farther into the screen; higher values pull it forward.",
        1, 100, 1, 0, false, 20
    );
    config.add_number_option(
